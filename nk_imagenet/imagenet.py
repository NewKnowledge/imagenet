    ...
    def __init__(self, model="inception_v3", include_top=False, pooling=None):
        ...
        if model == "xception":
            self.model = xception.Xception(
                weights="imagenet", include_top=include_top, pooling=pooling
            )
    ...
    def predict(self, images_array):
        ...
        if input_dim == 3:
            images_array = images_array[
                None, :, :, :
            ]  # add a dimension, making batch size 1
        elif input_dim != 4:
            raise Exception(
...
class ImagenetFeaturizer(ImagenetModel):
    def __init__(self, model="inception_v3", pooling=None, n_channels=None):
        ...
        self.output_shape = (
            (*output.shape[1:3], n_channels) if n_channels else output.shape[1:]
        )
    ...
    def get_features(self, images_array, flatten=True):
        ...
        if self.n_channels:
            logger.debug(
                f"truncating last dimension of array to only use the first {self.n_channels} channels"
            )
...
class ImagenetEverything:
    ...
    def __init__(
        ...
        if cache_dir:
            ...
            config = [
                f"objects-{self.n_objects}" if include_top else "features",
                str(cache_size),
                model,
    ...
    def get_features_from_url_batch(self, image_urls, ignore_failed=True):
        ...
        new_urls, cached_urls = partition(
            lambda x: x in self.cache, image_urls, as_list=True
        )
        logger.info(
