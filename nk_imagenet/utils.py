...
requests_session = (
    requests.Session() if os.environ.get("USE_REQUESTS_SESSION") == "True" else requests
)
...
def strip_alpha_channel(image):
    ...
    background = Image.new(
        image.mode[:-1], image.size, "#ffffff"
    )  # TODO is filling with black here a good idea?
...
def load_image_url(url, target_size=None):
    ...
    with Image.open(io.BytesIO(response.content)) as img:
        ...
        if target_size:
            img = img.resize(
                target_size
            )  # TODO use interpolation to downsample? (e.g. PIL.Image.LANCZOS)
